      ...
      try {
        ...
        try {
          ...
          if (data && data.filteredTexts) {
            ...
            setWebsiteData(`AI response to: ${fullPrompt}\n\nScraped content:\n${websiteDataText}`);
          } else {
          ...
          <Popover open={open} onOpenChange={setOpen}>
            ...
            <PopoverContent className="w-full p-0 bg-[#ffffff] border-[#0257AC]">
              <Command className="border-[#0257AC]">
                <CommandInput placeholder="Search preset..." className="text-white h-9" />
                <CommandList>
                  <CommandEmpty className="text-[#0257AC]">No preset found.</CommandEmpty>
                  <CommandGroup>
                    {presets.map((preset) => (
                      <CommandItem
                        key={preset.value}
                      ...
                      >
                        ...
                        <CheckIcon
                          className={cn(
                            'ml-auto h-4 w-4',
                            command === preset.label ? 'opacity-100' : 'opacity-0',
                          )}
                        />
                      </CommandItem>
                    ))}
        ...
        {isLoading ? (
          ...
          <div className="flex items-center justify-center h-full">
            <p className="text-center text-sm text-[#0257AC]">
              Select a preset or enter a custom prompt to get started with SparkMind.
            </p>
          </div>
        )}
      </ScrollArea>
