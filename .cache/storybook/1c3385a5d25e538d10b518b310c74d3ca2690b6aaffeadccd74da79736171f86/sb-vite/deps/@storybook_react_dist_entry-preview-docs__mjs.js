...
import { __commonJS, __esm, __export, __toCommonJS, __toESM } from './chunk-LK32TJAX.js';
...
var require_factoryWithTypeCheckers = __commonJS({
  'node_modules/.pnpm/prop-types@15.8.1/node_modules/prop-types/factoryWithTypeCheckers.js'(
    ...
    module.exports = (isValidElement4, throwOnDirectAccess) => {
...
var require_estraverse = __commonJS({
  'node_modules/.pnpm/estraverse@5.3.0/node_modules/estraverse/estraverse.js'(exports) {
    (function clone(exports2) {
      ...
      exports2.cloneEnvironment = () => clone({});
      return exports2;
    })(exports);
  },
});
    ...
    BasicSourceMapConsumer.prototype.computeColumnSpans =
      function SourceMapConsumer_computeColumnSpans() {
        for (var index = 0; index < this._generatedMappings.length; ++index) {
          ...
          mapping.lastGeneratedColumn = Number.POSITIVE_INFINITY;
        }
      };
    ...
    (() => {
      ...
      function updateDeeply(target, override) {
        ...
        function isHashObject(target2) {
          return (
            typeof target2 === 'object' && target2 instanceof Object && !(target2 instanceof RegExp)
          );
        }
        for (key in override) {
          if (override.hasOwnProperty(key)) {
        ...
        if (stmt.trailingComments) {
          if (preserveBlankLines) {
            ...
            specialBase = stringRepeat(
              ' ',
              calculateSpaces(toSourceNodeWhenNeeded([base2, result, indent]).toString()),
            );
      ...
      CodeGenerator.prototype.maybeBlockSuffix = (stmt, result) => {
      ...
      CodeGenerator.prototype.generateFunctionParams = function (node) {
        ...
        if (
          ...
          result =
            node.type === Syntax.ArrowFunctionExpression ? [generateAsyncPrefix(node, false)] : [];
          result.push('(');
          if (node.defaults) {
            hasDefault = true;
          }
      ...
      CodeGenerator.prototype.generateIterationForStatement = function (operator, stmt, flags) {
        ...
        withIndent(() => {
          ...
          result = [
            join(result, this.generateExpression(stmt.right, Precedence.Assignment, E_TTT)),
            ')',
          ];
        });
      ...
      CodeGenerator.prototype.semicolon = (flags) => {
      ...
      CodeGenerator.Statement = {
        ...
        ExportDefaultDeclaration: function (stmt, flags) {
          ...
          if (isStatement(stmt.declaration)) {
            ...
            result = join(
              result,
              this.generateExpression(stmt.declaration, Precedence.Assignment, E_TTT) +
                this.semicolon(flags),
            );
          }
          return result;
            ...
            if (stmt.specifiers.length === 0) {
              ...
              withIndent((indent2) => {
                ...
                for (i = 0, iz = stmt.specifiers.length; i < iz; ++i) {
                  result.push(indent2);
                  result.push(
                    this.generateExpression(stmt.specifiers[i], Precedence.Sequence, E_TTT),
                  );
                  if (i + 1 < iz) {
                    result.push(',' + newline);
                  }
        ...
        ExpressionStatement: function (stmt, flags) {
          ...
          function isClassPrefixed(fragment2) {
            ...
            return (
              code === 123 || esutils.code.isWhiteSpace(code) || esutils.code.isLineTerminator(code)
            );
          }
        ...
        SwitchCase: function (stmt, flags) {
          ...
          withIndent(() => {
            if (stmt.test) {
              result = [
                join('case', this.generateExpression(stmt.test, Precedence.Sequence, E_TTT)),
                ':',
              ];
            } else {
              result = ['default:'];
            ...
            for (; i < iz; ++i) {
              ...
              fragment = addIndent(this.generateStatement(stmt.consequent[i], bodyFlags));
              result.push(fragment);
              if (
                i + 1 !== iz &&
      ...
      CodeGenerator.Expression = {
        ...
        CallExpression: function (expr, precedence, flags) {
          ...
          for (i = 0, iz = expr['arguments'].length; i < iz; ++i) {
            result.push(
              this.generateExpression(expr['arguments'][i], Precedence.Assignment, E_TTT),
            );
            if (i + 1 < iz) {
              result.push(',' + space);
            }
          ...
          withIndent((indent2) => {
            ...
            for (i = 0, iz = expr.elements.length; i < iz; ++i) {
              if (!expr.elements[i]) {
                ...
                result.push(
                  this.generateExpression(expr.elements[i], Precedence.Assignment, E_TTT),
                );
              }
              if (i + 1 < iz) {
          ...
          withIndent((indent2) => {
            ...
            if (multiline) {
              ...
              for (i = 1, iz = expr.properties.length; i < iz; ++i) {
                result.push(indent2);
                result.push(
                  this.generateExpression(expr.properties[i], Precedence.Sequence, E_TTT),
                );
                if (i + 1 < iz) {
                  result.push(',' + newline);
                }
        ...
        ObjectPattern: function (expr, precedence, flags) {
          ...
          withIndent((indent2) => {
            ...
            for (i2 = 0, iz2 = expr.properties.length; i2 < iz2; ++i2) {
              result.push(multiline ? indent2 : '');
              result.push(this.generateExpression(expr.properties[i2], Precedence.Sequence, E_TTT));
              if (i2 + 1 < iz2) {
                result.push(',' + (multiline ? newline : space));
              }
            }
        ...
        ThisExpression: (expr, precedence, flags) => 'this',
        Super: (expr, precedence, flags) => 'super',
        Identifier: (expr, precedence, flags) => generateIdentifier(expr),
        ImportDefaultSpecifier: (expr, precedence, flags) =>
          generateIdentifier(expr.id || expr.local),
        ...
        TemplateElement: (expr, precedence, flags) => expr.value.raw,
  ...
  'node_modules/.pnpm/acorn@7.4.1/node_modules/acorn/dist/acorn.mjs'() {
    ...
    pp$1.parseFunction = function (node, statement, allowExpressionBody, isAsync) {
      ...
      if (statement & FUNC_STATEMENT) {
        node.id =
          statement & FUNC_NULLABLE_ID && this.type !== types.name ? null : this.parseIdent();
    ...
    pp$1.parseClassElement = function (constructorAllowsSuper) {
      ...
      var tryContextual = (k, noLineBreak) => {
        ...
        if (this.type !== types.parenL && (!noLineBreak || !this.canInsertSemicolon())) {
          return true;
        }
        if (method.key) {
          this.unexpected();
    ...
    pp$2.parseBindingListItem = (param) => param;
    ...
    pp$7.initialContext = () => [types$1.b_stat];
    ...
    pp$8.regexp_eatSyntaxCharacter = (state) => {
    ...
    pp$8.regexp_eatPatternCharacters = (state) => {
      var start = state.pos;
      var ch = 0;
    ...
    pp$8.regexp_eatExtendedPatternCharacter = (state) => {
      var ch = state.current();
      if (
        ch !== -1 &&
        ch !== 36 &&
    ...
    pp$8.regexp_eatControlEscape = (state) => {
      var ch = state.current();
      if (ch === 116) {
    ...
    pp$8.regexp_eatControlLetter = (state) => {
      var ch = state.current();
    ...
    pp$8.regexp_eatDecimalEscape = (state) => {
      state.lastIntValue = 0;
      var ch = state.current();
      if (ch >= 49 && ch <= 57) {
        do {
    ...
    pp$8.regexp_validateUnicodePropertyNameAndValue = (state, name, value) => {
    ...
    pp$8.regexp_validateUnicodePropertyNameOrValue = (state, nameOrValue) => {
      if (!state.unicodeProperties.binary.test(nameOrValue)) {
        state.raise('Invalid property name');
      }
    };
    pp$8.regexp_eatUnicodePropertyName = (state) => {
      var ch = 0;
      state.lastStringValue = '';
    ...
    pp$8.regexp_eatUnicodePropertyValue = (state) => {
      var ch = 0;
      state.lastStringValue = '';
    ...
    pp$8.regexp_eatClassControlLetter = (state) => {
      var ch = state.current();
    ...
    pp$8.regexp_eatDecimalDigits = (state) => {
      var start = state.pos;
      var ch = 0;
      state.lastIntValue = 0;
    ...
    pp$8.regexp_eatHexDigits = (state) => {
      var start = state.pos;
      var ch = 0;
      state.lastIntValue = 0;
    ...
    pp$8.regexp_eatOctalDigit = (state) => {
      var ch = state.current();
      if (isOctalDigit(ch)) {
    ...
    pp$8.regexp_eatFixedHexDigits = (state, length) => {
      var start = state.pos;
      state.lastIntValue = 0;
    ...
    pp$9.readInt = function (radix, len, maybeLegacyOctalNumericLiteral) {
      ...
      var start = this.pos,
        total = 0,
        lastCode = 0;
      for (var i = 0, e = len == null ? Number.POSITIVE_INFINITY : len; i < e; ++i, ++this.pos) {
        var code = this.input.charCodeAt(this.pos),
          val = void 0;
      ...
      switch (ch) {
        ...
        default:
          if (ch >= 48 && ch <= 55) {
            ...
            var octal = Number.parseInt(octalStr, 8);
            if (octal > 255) {
          ...
          while (this.pos < this.input.length && count++ < 10) {
            ...
            if (ch === ';') {
              if (str[0] === '#') {
                if (str[1] === 'x') {
                  str = str.substr(2);
                  if (hexNumber.test(str)) entity = String.fromCharCode(Number.parseInt(str, 16));
                } else {
                  str = str.substr(1);
                  if (decimalNumber.test(str))
                    entity = String.fromCharCode(Number.parseInt(str, 10));
                }
              } else {
      ...
      if (isObj(input)) {
        ...
        if (combinedOptions.filter) {
          objKeys_1 = objKeys_1.filter(
            (el) => combinedOptions.filter && combinedOptions.filter(input, el),
          );
        }
...
var parseReactElement = function parseReactElement2(element, options) {
  var _options$displayName = options.displayName,
    displayNameFn =
      _options$displayName === void 0 ? getReactElementDisplayName : _options$displayName;
  ...
  var childrens = import_react.default.Children.toArray(element.props.children)
    ...
    .map((child) => parseReactElement2(child, options));
...
var formatComplexDataStructure = (value, inline, lvl, options) => {
...
var formatProp = (name, hasValue, value, hasDefaultValue, defaultValue, inline, lvl, options) => {
  if (!hasValue && !hasDefaultValue) {
    throw new Error(
      'The prop "'.concat(name, '" has no value and no default: could not be formatted'),
    );
  }
...
var mergeSiblingPlainStringChildrenReducer = (previousNodes, currentNode) => {
...
var sortPropsByNames = (shouldSortUserProps) => (props) => {
  var haveKeyProp = props.includes('key');
  var haveRefProp = props.includes('ref');
  var userPropsOnly = props.filter((oneProp) => !isKeyOrRefProps(oneProp));
...
var formatReactElementNode = (node, inline, lvl, options) => {
  var type = node.type,
...
var formatReactFragmentNode = (node, inline, lvl, options) => {
  var type = node.type,
    key = node.key,
    childrens = node.childrens;
...
var formatTreeNode = (node, inline, lvl, options) => {
  if (node.type === 'number') {
...
var formatTree = (node, options) => formatTreeNode(node, false, 0, options);
...
function parseJsxElement(jsxElementNode) {
  const inferredType = { type: 'Element' },
...
function generateCustom({ raw }) {
  return raw != null
    ...
    : createTypeDef({ name: 'custom', short: CUSTOM_CAPTION, compact: CUSTOM_CAPTION });
}
function generateFunc(extractedProp) {
  const { jsDocTags } = extractedProp;
  return jsDocTags != null && (jsDocTags.params != null || jsDocTags.returns != null)
    ? createTypeDef({
        name: 'func',
    ...
    : createTypeDef({ name: 'func', short: FUNCTION_CAPTION, compact: FUNCTION_CAPTION });
}
...
function generateReactObject(rawDefaultProp) {
  ...
  if ((0, import_isString.default)(type) && isHtmlTag(type)) {
    const jsxSummary = reactElementToJsxString(rawDefaultProp, { tabStop: 0 }).replace(
      /\r?\n|\r/g,
      '',
    );
  ...
  if ((0, import_isFunction.default)(rawDefaultProp.render)) isElement = true;
    ...
    try {
      ...
      hasParams
        ...
        : (innerElement = rawDefaultProp()),
        innerElement != null && isReactElement(innerElement) && (isElement = true);
    } catch {}
  }
...
function getPropDefs(component, section) {
  ...
  const extractedProps = Bw(processedComponent, section);
...
function simplifyNodeForStringify(node) {
  if ((0, import_react2.isValidElement)(node)) {
    const props = Object.keys(node.props).reduce(
...
var skipJsxRender = (context) => {
  var _a;
  const sourceParams =
      (_a = context == null ? void 0 : context.parameters.docs) == null ? void 0 : _a.source,
...
var isMdx = (node) => {
  var _a, _b;
  return (
    ((_a = node.type) == null ? void 0 : _a.displayName) === 'MDXCreateElement' &&
    !!((_b = node.props) == null ? void 0 : _b.mdxType)
  );
};
var mdxToJsx = (node) => {
...
var applyDecorators = (storyFn, decorators2) => {
  const jsxIndex = decorators2.findIndex((d) => d.originalFn === jsxDecorator),
    reorderedDecorators =
      jsxIndex === -1 ? decorators2 : [...decorators2.splice(jsxIndex, 1), ...decorators2];
  return (0, import_preview_api.defaultDecorateStory)(storyFn, reorderedDecorators);
};
var parameters = {
  docs: { story: { inline: true }, extractArgTypes, extractComponentDescription: $w },
};
