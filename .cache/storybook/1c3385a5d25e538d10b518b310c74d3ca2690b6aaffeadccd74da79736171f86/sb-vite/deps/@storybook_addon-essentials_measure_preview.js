...
function getDocumentWidthAndHeight() {
  let container = import_global.global.document.documentElement,
...
function destroy() {
  ...
  state.canvas &&
    (clear(),
    (_a = state.canvas.parentNode) == null
      ? void 0
      : _a.removeChild(state.canvas),
    (state = {}))
}
var colors = {
  margin: "#f6b26b",
...
function labelStacks(context, measurements, labels, externalLabels) {
  let stacks = labels.reduce((acc, l) => {
    var _a
    return (
      Object.prototype.hasOwnProperty.call(acc, l.position) ||
        (acc[l.position] = []),
...
function floatingAlignment(extremities) {
  let windowExtremities = {
      ...
      bottom:
        import_global.global.window.scrollY +
        import_global.global.window.innerHeight,
      left: import_global.global.window.scrollX,
      right:
        import_global.global.window.scrollX +
        import_global.global.window.innerWidth
    },
    distances = {
...
function drawPadding(
  ...
  let paddingWidth = width - border.left - border.right,
    paddingHeight =
      height - padding.top - padding.bottom - border.top - border.bottom
...
function drawBoxModel(element) {
  return (context) => {
    if (element && context) {
      let measurements = measureElement(element),
...
var deepElementFromPoint = (x, y) => {
  let element = import_global.global.document.elementFromPoint(x, y),
    crawlShadows = (node) => {
