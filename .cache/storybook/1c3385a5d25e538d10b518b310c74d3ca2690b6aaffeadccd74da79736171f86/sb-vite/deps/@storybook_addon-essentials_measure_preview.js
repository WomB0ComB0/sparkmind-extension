...
function createCanvas() {
  const canvas = import_global.global.document.createElement('canvas');
  ...
  return (
    ...
    import_global.global.document.body.appendChild(canvas),
    { canvas, context, width, height }
  );
}
function setCanvasWidthAndHeight(canvas, context, { width, height }) {
  (canvas.style.width = `${width}px`), (canvas.style.height = `${height}px`);
  const scale = import_global.global.window.devicePixelRatio;
...
function destroy() {
  ...
  state.canvas &&
    (clear(),
    (_a = state.canvas.parentNode) == null ? void 0 : _a.removeChild(state.canvas),
    (state = {}));
}
var colors = {
  margin: '#f6b26b',
...
function positionCoordinate(position, { padding, border, width, height, top, left }) {
  let contentWidth = width - border.left - border.right - padding.left - padding.right,
    ...
    position === 'top'
      ...
      : position === 'right'
        ...
        : position === 'bottom'
          ...
          : position === 'left'
            ? (y += contentHeight / 2)
            : position === 'center' && ((x += contentWidth / 2), (y += contentHeight / 2)),
    { x, y }
  );
}
function offset(type, position, { margin, border, padding }, labelPaddingSize, external) {
  let shift = (dir) => 0,
    ...
    labelPaddingShift = external ? labelPaddingSize * 2 : 0;
  return (
    type === 'padding'
    ...
    position === 'top'
      ...
      : position === 'right'
        ...
        : position === 'bottom'
          ...
          : position === 'left' && (offsetX = -shift('left')),
    { offsetX, offsetY }
  );
}
...
function drawLabel(
  ...
  let { x, y } = positionCoordinate(position, measurements),
    { offsetX, offsetY } = offset(type, position, measurements, labelPadding + 1, external);
  (x += offsetX), (y += offsetY);
...
function drawFloatingLabel(context, measurements, { type, text }) {
  let { floatingAlignment: floatingAlignment2, extremities } = measurements,
...
function labelStacks(context, measurements, labels, externalLabels) {
  const stacks = labels.reduce((acc, l) => {
    ...
    return (
      ...
      (_a = acc[l.position]) == null ? void 0 : _a.push(l),
      acc
    );
  }, {});
...
function measureElement(element) {
  let style = import_global.global.getComputedStyle(element),
    { top, left, right, bottom, width, height } = element.getBoundingClientRect(),
    {
      marginTop,
      marginBottom,
      marginLeft,
  ...
  (top = top + import_global.global.window.scrollY),
    (left = left + import_global.global.window.scrollX),
    (bottom = bottom + import_global.global.window.scrollY),
    (right = right + import_global.global.window.scrollX);
  const margin = {
  ...
  return (context) => {
    if (element && context) {
      const measurements = measureElement(element),
        marginLabels = drawMargin(context, measurements),
        paddingLabels = drawPadding(context, measurements),
        borderLabels = drawBorder(context, measurements),
        contentLabels = drawContent(context, measurements),
        externalLabels =
...
var deepElementFromPoint = (x, y) => {
  const element = import_global.global.document.elementFromPoint(x, y),
    crawlShadows = (node) => {
